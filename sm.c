...
/* Dump the contents of an SMState to a string. */
void otrl_sm_to_string(char * string, const OtrlSMState *sm)
{
    fprintf(f, "  SM state:\n");
    fprintf(f, "    Next expected: %d (%s)\n", sm->nextExpected,
            sm->nextExpected == OTRL_SMP_EXPECT1 ? "EXPECT1" :
...
/* Dump the contents of an SMState to a string. */
void otrl_sm_from_string(const char * string, OtrlSMState *sm)
{
    fprintf(f, "  SM state:\n");
    fprintf(f, "    Next expected: %d (%s)\n", sm->nextExpected,
            sm->nextExpected == OTRL_SMP_EXPECT1 ? "EXPECT1" :
...
gcry_mpi_t get_sm_generator(void)
{
    return SM_GENERATOR;
}

int get_sm_mod_len_bits(void)
{
    return SM_MOD_LEN_BITS;
}
